...
test("front-page", async () => {
  ...
  expect(wrapper.find("#front-page-title").text()).toBe(
    "The best solution for your productions"
  );
  expect(wrapper.find("#front-page-description").text()).toBe(
  ...
  expect(wrapper.find('input[name="username"]').element.value).toBe(
    "testestest"
  ),
...
test("featured-component", async () => {
  ...
  expect(wrapper.find("#featured-description-1").text()).toBe(
    "Laudantium totam quas cumque pariatur at doloremque hic quos quia eius. Reiciendis optio minus mollitia rerum labore facilis inventore voluptatem ad, quae quia sint. Ullam."
  );
  expect(wrapper.find("#featured-benefit-2").text()).toBe("Safe and solid");
  expect(wrapper.find("#featured-description-2").text()).toBe(
    "Eum nostrum fugit numquam, voluptates veniam neque quibusdam ullam aspernatur odio soluta, quisquam dolore animi mollitia a omnis praesentium, expedita nobis!"
  );
  expect(wrapper.find("#featured-benefit-3").text()).toBe("Extremely fast");
  expect(wrapper.find("#featured-description-3").text()).toBe(
    "Enim cupiditate, minus nulla dolor cumque iure eveniet facere ullam beatae hic voluptatibus dolores exercitationem? Facilis debitis aspernatur amet nisi?"
  );
});
...
test("pricing-component", async () => {
  ...
  expect(wrapper.find("#pricing-description").text()).toBe(
    "Lorem ipsum dolor sit amet, consectetur adipisicing elit. A soluta corporis voluptate ab error quam dolores doloremque, quae consectetur."
  );
...
test("content-horizontal", async () => {
  ...
  expect(wrapper.find("#content-horizontal-title").text()).toBe(
    "Latest Scenes"
  );
  expect(wrapper.find("#trip-title-1").text()).toBe("My paradise");
  expect(wrapper.find("#trip-date-1").text()).toBe(
    "Published 13.01.2022 by Anna Maria Doe"
  );
  expect(wrapper.find("#trip-description-1").text()).toBe(
  ...
  expect(wrapper.find("#trip-date-2").text()).toBe(
    "Published 12.01.2022 by Halley Frank"
  );
  expect(wrapper.find("#trip-description-2").text()).toBe(
  ...
  expect(wrapper.find("#trip-date-3").text()).toBe(
    "Published 10.01.2022 by Joe Svan"
  );
  expect(wrapper.find("#trip-description-3").text()).toBe(
