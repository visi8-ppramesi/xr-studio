...
const signMessage = async (message, privKey, pass = null) => {
    ...
    const keyObject = await webcrypto.subtle.importKey('jwk', privKey, { name: "ECDSA", namedCurve: "P-384" }, true, ['sign'])
    const signature = await webcrypto.subtle.sign({ name: "ECDSA", hash: { name: "SHA-384" } }, keyObject, encoded)
...
const verifySignature = async (message, signature, publicKey) => {
    ...
    const encoded = enc.encode(message)
    const keyObject = await webcrypto.subtle.importKey('jwk', publicKey, { name: "ECDSA", namedCurve: "P-384" }, true, ['verify'])
    const verified = await webcrypto.subtle.verify({ name: "ECDSA", hash: { name: "SHA-384" } }, keyObject, signature, encoded)
    return verified
}
...
const generateKey = (pass) => new Promise((resolve, reject) => {
    webcrypto.subtle.generateKey(
        ...
        const publicPromise = webcrypto.subtle.exportKey('jwk', keyPair.publicKey).then((pubKey) => {
            return Buffer.from(JSON.stringify(pubKey)).toString('base64')
        })
        const privatePromise = webcrypto.subtle.exportKey('jwk', keyPair.privateKey).then((pKey) => {
